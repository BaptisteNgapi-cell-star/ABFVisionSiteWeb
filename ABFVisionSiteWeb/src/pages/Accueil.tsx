...
const initialTestimonialsData = [
  {
    ...
    comment: {
      fr: "ABFVision a transformé notre façon de gérer l'établissement. L'interface est intuitive et le gain de temps considérable !",
      en: "ABFVision transformed how we manage our institution. The interface is intuitive and the time savings are considerable!"
    },
    rating: 9,
  ...
  return (
    <div className="flex items-center gap-1">
      {[...Array(5)].map((_, i) => {
        ...
        const isHalf = !isFull && starValue - 1 <= displayRating;
        ...
        return (
          ...
          >
            {isFull ? (
              <FiStar className="text-yellow-400 fill-yellow-400" size={size} />
            ) : isHalf ? (
              <div className="relative">
                <FiStar className="text-gray-300" size={size} />
...
const TestimonialCard: React.FC<{ 
  testimonial: typeof initialTestimonialsData[0] | Comment; 
  language: 'fr' | 'en';
  isActive?: boolean;
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[#9AAEFF] text-sm font-light">
              {typeof testimonial.role === 'object' ? testimonial.role[language] : testimonial.role}
            </p>
          </div>
        </div>
      ...
      <p className="text-[#98A2B3] font-light leading-relaxed text-sm">
        "{typeof testimonial.comment === 'object' ? testimonial.comment[language] : testimonial.comment}"
      </p>
...
const CommentModal: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (comment: Omit<Comment, 'id' | 'userId' | 'date'>) => void;
  language: 'fr' | 'en';
  ...
  if (isOpen && formData.name === '' && formData.role === '' && formData.comment === '') {
    setFormData({
      name: '',
      role: '',
      comment: '',
...
// Loader avec les couleurs ABFVision
...
// Points scintillants prédéfinis pour éviter Math.random()
...
const Homepage = () => {
  ...
  const [language, setLanguage] = useState<'fr' | 'en'>(() => {
    if (typeof window !== 'undefined') {
      const savedLanguage = localStorage.getItem('abfvision-language') as 'fr' | 'en';
      return savedLanguage || 'fr';
    }
    return 'fr';
  });

  // États pour l'authentification et les commentaires
  const [user, setUser] = useState<User>(() => {
    if (typeof window !== 'undefined') {
      const savedUser = localStorage.getItem('abfvision-user');
  ...
  const [userComments, setUserComments] = useState<Comment[]>(() => {
    if (typeof window !== 'undefined') {
      const savedComments = localStorage.getItem('abfvision-user-comments');
      return savedComments ? JSON.parse(savedComments) : [];
    }
    return [];
  });

  const [allTestimonials, setAllTestimonials] = useState([...initialTestimonialsData]);
  ...
  const toggleLanguage = () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('abfvision-language', newLanguage);
    }
  };
  ...
  const handleLogin = () => {
    const newUser: User = {
      id: 'user-' + Date.now(),
      name: 'Utilisateur ABFVision',
      email: 'user@abfvision.com',
      isLoggedIn: true
    };
    setUser(newUser);
    if (typeof window !== 'undefined') {
      localStorage.setItem('abfvision-user', JSON.stringify(newUser));
    }
  ...
  const handleLogout = () => {
    ...
    setUser(guestUser);
    if (typeof window !== 'undefined') {
      localStorage.setItem('abfvision-user', JSON.stringify(guestUser));
    }
    setIsProfileMenuOpen(false);
  ...
  const handleRegister = () => {
    // Pour l'instant, l'inscription fonctionne comme la connexion
    handleLogin();
  };

  // Gestion des commentaires
  const handleAddComment = async (commentData: Omit<Comment, 'id' | 'userId' | 'date'>) => {
    if (!user.isLoggedIn) {
      handleLogin();
      return;
    }
    ...
    // Ajouter aux témoignages publics
    ...
    setAllTestimonials(prev => [publicTestimonial, ...prev]);
    
    if (typeof window !== 'undefined') {
      localStorage.setItem('abfvision-user-comments', JSON.stringify(updatedComments));
    }
  ...
  // Sections avec palette ABFVision
  const sections = [
    { 
      id: 'accueil', 
  ...
  return (
    <div className="min-h-screen bg-[#1E2430] text-white overflow-x-hidden">
      ...
      {/* Navigation ABFVision - Version épurée */}
      <motion.nav
        initial={{ y: -100 }}
        animate={{ y: 0 }}
...
// Hero Section ABFVision avec témoignages améliorée
interface HeroSectionProps {
  onNavigate: (section: string) => void;
  language: 'fr' | 'en';
  testimonials: (typeof initialTestimonialsData[0] | Comment)[];
  ...
  return (
    <section className="min-h-screen relative overflow-hidden bg-gradient-to-br from-[#1E2430] via-[#1A1F2E] to-[#1E2430] pt-16">
      ...
      <div className="absolute inset-0">
        ...
        {/* Points scintillants avec positions prédéfinis */}
      ...
      <div className="relative z-10 min-h-screen flex items-center">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            {/* Colonne de droite - Témoignages avec boutons intégrés */}
            <motion.div
              initial={{ opacity: 0, x: 50 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              ...
              <div className="relative">
                <div className="w-full bg-gradient-to-br from-[#9AAEFF]/5 to-[#9AAEFF]/10 rounded-2xl border border-[#9AAEFF]/20 backdrop-blur-sm p-8">
                  ...
                  {/* En-tête témoignages avec boutons d'action */}
                  <div className="text-center mb-8">
                    ...
                    {/* Boutons d'action intégrés dans l'en-tête */}
                    <div className="flex flex-col sm:flex-row gap-3 justify-center">
                      ...
                      {isMyCommentsView ? (
                        ...
                        <>
                          ...
                          {/* Bouton Mes commentaires (si connecté) */}
                  ...
                  {testimonials.length > 0 ? (
                    <>
                      {/* Témoignage actuel (pour le carousel) ou liste (pour mes commentaires) */}
                      {!isMyCommentsView ? (
                        <AnimatePresence mode="wait">
                          <TestimonialCard
                      ...
                      {/* Indicateurs de témoignages (uniquement pour le carousel) */}
                      ...
                      {/* Note moyenne (uniquement pour les témoignages publics) */}
                      {!isMyCommentsView && (
                        <motion.div
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
